...
export class CustomersService {
  ...
  async create(createCustomerDto: CreateCustomerDto): Promise<Customer> {
    ...
    // If customer is an owner and no ownerUuid is specified, set themselves as owner
    if (savedCustomer.isOwner && !savedCustomer.ownerUuid) {
      savedCustomer.ownerUuid = savedCustomer.uuid;
  ...
  async findAllPaginated(paginationDto: PaginationDto, currentCustomer?: Customer): Promise<PaginatedResponse<Customer & { role?: string }>> {
    const { page, limit } = paginationDto;
    return await this.cacheService.caching(
      CUSTOMER_TABLE_NAME,
      { page, limit, roleAccess: currentCustomer?.uuid },
      async () => {
  ...
  async findOne(uuid: string, withRole: boolean = false): Promise<Customer | (Customer & { role?: string })> {
    return await this.cacheService.caching(
      CUSTOMER_TABLE_NAME,
      { id: uuid, withRole },
      async () => {
        ...
        if (!customer) {
          throw new NotFoundException(`Customer with ID ${uuid} not found`);
        }
  ...
  async update(uuid: string, updateCustomerDto: UpdateCustomerDto): Promise<Customer> {
    const customer = await this.findOne(uuid);
  ...
  async findByOwner(ownerUuid: string): Promise<Customer[]> {
    return await this.cacheService.caching(
      CUSTOMER_TABLE_NAME,
      { ownerUuid },
      async () => {
